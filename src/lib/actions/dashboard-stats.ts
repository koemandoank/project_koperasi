  ...
  return remember("stats:admin", 300, async () => {
    try {
      ...
      const [
        ...
        prisma.member.count({ where: { created_at: { gte: firstDayOfMonth } } }),
        ...
        prisma.stock_transfer_orders.count({
        ...
        prisma.journal_lines.aggregate({
          ...
          where: {
            ...
            journal_entries: { is_posted: true, entry_date: { gte: firstDayOfYear } }
          }
        }),
        prisma.journal_lines.aggregate({
          _sum: { debit: true, credit: true },
      ...
      const assetLiquidity = Number(assetJournalLines._sum.debit || 0) - Number(assetJournalLines._sum.credit || 0)
  ...
  return remember("stats:kredit", 300, async () => {
    try {
      ...
      const [
        ...
        prisma.saving_transactions.aggregate({
          _sum: { amount: true },
  ...
  return remember("stats:kasir", 60, async () => {
    try {
      ...
      if (lowStockIds.length > 0) {
        activePOs = await prisma.purchase_orders.findMany({
          where: {
