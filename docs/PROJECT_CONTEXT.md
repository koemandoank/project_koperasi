...
- **Rollback & Protections (Payroll salary cut after May 2026)**:
  - Membersihkan dan membatalkan 83 record data cicilan yang terbayar di muka setelah **31 Mei 2026** (periode Juni & Juli 2026), mengembalikan outstanding pinjaman, total terbayar, dan memulihkan status `loan_schedules` kembali ke `"pending"`.
  - Menyelaraskan nominal jurnal manual penerimaan kas harian (ID Jurnal 9) dari **Rp 225.843.000** menjadi **Rp 123.757.000** (sesuai data riil terbayar hari ini pasca rollback).
  - Mengimplementasikan helper `isPayableThisMonth` di [kelola-pinjaman-client.tsx](file:///d:/laragon/www/koperasi-sulfindo/src/app/(dashboard)/pinjaman/kelola-pinjaman-client.tsx) untuk menyembunyikan checkbox pembayaran massal bagi pinjaman yang kewajiban angsurannya sudah terbayar pada bulan berjalan.
  - Menambahkan dialog konfirmasi (`window.confirm`) pada proses pencatatan angsuran tunggal jika kasir mencoba mendahului pembayaran angsuran untuk bulan-bulan mendatang.